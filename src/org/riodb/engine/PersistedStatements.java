...
public class PersistedStatements {
	...
	public void dropStreamStmt(String name) {
		...
		System.out.println("Persisted streams. Before removal: ");
		...
		System.out.println("Persisted streams. After removal: ");
		for (Entry<String, String> entry : streamStatements.entrySet()) {
			System.out.println(entry.getValue());
		}
	...
	private boolean createPersistedStmtFile() {
		if (!fileExists) {
			...
			try {
				...
				RioDB.rio.getSystemSettings().getLogger().debug("File to persist statements created as '"+ RioDB.rio.getSystemSettings().getPersistedStmtFile() +"'");
			} catch (IOException e) {
	...
	private void updatePersistedStmtFile() {
			...
			if (fileExists || createPersistedStmtFile()) {
				...
				try {
					...
					RioDB.rio.getSystemSettings().getLogger().debug("Statement persisted to local file '"+ RioDB.rio.getSystemSettings().getPersistedStmtFile() +"'");
