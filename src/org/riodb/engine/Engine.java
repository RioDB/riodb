...
public class Engine {
	...
	public boolean addStream(Stream newStream) throws ExceptionSQLStatement, RioDBPluginException {
		...
		RioDB.rio.getSystemSettings().getLogger().debug("Engine.addStream. added.");
		return true;
	}
	...
	public int getStreamIdOfWindow(String windowName) {
		...
		for (int i = 0; i < streams.length; i++) {
			if (streams[i] != null && streams[i].getWindowMgr().hasWindow(windowName))
				return i;
		}
		return -1;
	}
	...
	public String listAllQueries() {
		String response = "[ ";
		boolean previousStreamHadQueryes = false;
		for (int i = 0; i < streams.length; i++) {
			if (streams[i] != null) {
				if (previousStreamHadQueryes) {
					response = response + ",";
				}
	...
	/*
	 * // print counters for debugging public void printMsgCounters() { String s =
	 * ""; for (int i = 0; i < streams.length; i++) { s = "\t" + "stream[" + i +
	 * "]: " + streams[i].getMsgCounter(); } System.out.println(s); }
	 */

	/*
	 * remove a stream from array we are not resizing the array. Only setting the
	 * decommissioned stream slot to null.
	 */
	...
	public boolean start(int streamId) throws RioDBPluginException {
		if (!online) {
			return false;
		}
		if (streamId < streams.length && streamId >= 0 && streams[streamId] != null) {
	...
	/*
	 * Drop a query... We don't really now the address of a query here. So we loop
	 * through all Streams asking each of them to drop the query if they find it.
	 * TODO: Possible enhancement to include StreamID has part of query ID: 1.5
	 * would mean that query #5 belongs in Stream #1 Then, no more need for looping.
	 */
