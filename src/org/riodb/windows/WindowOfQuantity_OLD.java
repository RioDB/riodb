...
public class WindowOfQuantity_OLD implements Window {
	...
	// Constructor
	public WindowOfQuantity_OLD(int range, boolean[] functionsRequired, int partitionExpiration) {
	...
	public Window makeFreshClone() {
		return new WindowOfQuantity_OLD(range, required_Functions, partitionExpiration);
	}
	...
	private void add(double elementInserted) {
		...
		if (windowSummary.isFull()) {
			...
			if (requiresSlope) {
				...
				if (slopeExy > 1e10 || slopeEx2 > 1e10) {
				//if ( slopeInsertsSinceReset > 10000) {
					...
					slopeExy = slopeExy + (slopeInsertsSinceReset * elementInserted) - (removedIndex * elementEvicted);
					// Update the sum of x square
					slopeEx2 = slopeEx2 + (slopeInsertsSinceReset * slopeInsertsSinceReset)
				...
				if(windowSummary.getCount() > 1 && windowSummary.getSlope() != 1) {
					System.out.println("				last = " + elementInserted + 
...
//		System.out.println(s);
	...
	private void resetSlopeVars() {
		...
		slopeEx2 = (slopeInsertsSinceReset * (slopeInsertsSinceReset + 1) * (2 * slopeInsertsSinceReset + 1)) / 6;
		...
		for (int i = 0; i < circularArray.length; i++) {
			...
			slopeExy = slopeExy + (i - sub + incr) * circularArray[i];
		}
