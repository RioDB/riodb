...
public class WindowOfTimeComplex_OLD implements Window {
...
//	private int minSiblings; // no longer needed
	...
	// Constructor
	public WindowOfTimeComplex_OLD(int range, boolean[] functionsRequired, int partitionExpiration) {
	...
	public Window makeFreshClone() {
		return new WindowOfTimeComplex_OLD(range, functionsRequired, partitionExpiration);
	}
	...
	private void add(double elementInserted, int currentSecond) {
		...
		if (arrayDeque.size() >= 1) {
			...
			if (requiresSlope) {
				...
				if ((slopeExy > 1e10 || slopeEx2 > 1e10) && insertsSinceSlopeReset > windowSummary.getCount()) {
					resetSlopeVars();
				} else {
					// count up the inserts since last slope reset.
					insertsSinceSlopeReset++;
					// sum of all X positions...
					slopeEx = (long) slopeEx + insertsSinceSlopeReset;
					// sum of x*y
					slopeExy = (double) slopeExy + (insertsSinceSlopeReset * elementInserted);
					// sum of x square
					slopeEx2 = (double) slopeEx2 + (insertsSinceSlopeReset * insertsSinceSlopeReset);
				}
...
//				printSorted(in);
			...
			if (requiresSlope) {
				...
				slopeEx = 1; // starting
				slopeExy = elementInserted; // x is 1. x*y = y
				slopeEx2 = 1; // starting
				windowSummary.setSlope(computeWindowSlope());
			}
		}
...
//								System.out.println("    ### Out: " + evictingElement.doubleValue() + " median: " + localMedian + " marker: " + medianMarker);
						...
						if (requiresSlope) {
							...
							slopeExy = slopeExy - (removedIndex * evictingElement.doubleValue());
							// Update the sum of x square
							slopeEx2 = slopeEx2 - (removedIndex * removedIndex);
	...
	private float computeWindowSlope() {
		float d = (float) ((arrayDeque.size() * slopeExy - slopeEx * windowSummary.getSum()) / (arrayDeque.size() * slopeEx2 - slopeEx * slopeEx));
		return (float) d;
	}
	...
	private void resetSlopeVars() {
		...
		slopeEx2 = (insertsSinceSlopeReset * (insertsSinceSlopeReset + 1) * (2 * insertsSinceSlopeReset + 1)) / 6;

		slopeExy = 0;
		int x = 1;
		for (ValueWithTimestamp f : arrayDeque) {
			slopeExy = slopeExy + (x++ * f.doubleValue());
		}
